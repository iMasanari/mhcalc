...
var attackAndAffinityUp = function (skill, value) {
...
function getWeaponLevelList(type, name) {
    return weaponData[type].list[name].list;
}
function getWeapon(type, name, level) {
...
function calc(weapon, activeSkillList) {
    var skill = {
        power: 0,
        mult: 1,
        affinity: 0
...
function getRanking(weapon, activeSkill) {
    var activeSkillList = skillNameList.filter(function (item) {
        return activeSkill[item.group] === item.name;
    });
    var orgPower = calc(weapon, activeSkillList);
    return skillNameList.map(function (item, index) {
        var isActive = activeSkill[item.group] === item.name;
        var result = {
            name: item.name,
            isActive: isActive,
        ...
        var activeSkillList = skillNameList.filter(function (simulateItem) {
            return activeSkill[simulateItem.group] === simulateItem.name && simulateItem.group !== item.group;
        });
        if (isActive) {
            var power = calc(weapon, activeSkillList);
...
var SkillRanking = (function (_super) {
    ...
    SkillRanking.prototype.render = function () {
        var _this = this;
        if (this.props.weapon.power == null)
            return null;
        var skillRanking = getRanking(this.props.weapon, this.props.activeSkill);
        var testSkillRanking = {};
        for (var i = skillRanking.length; i--;) {
            skillRanking[i].index = i;
            testSkillRanking[skillRanking[i].name] = skillRanking[i];
        }
        var TableRows = Object.keys(this.skillActionList).map(function (itemName, i) {
            var item = testSkillRanking[itemName];
            return React.createElement(TableRow, {key: item.name, item: item, action: _this.skillActionList[item.name]});
        });
        return React.createElement("div", {className: "SkillRanking"}, 
            React.createElement("table", {style: { height: 40 * (skillRanking.length + 1) }}, 
...
var TableRow = function (props) {
    return React.createElement("tr", {className: props.item.isActive ? 'checked' : '', onClick: props.action, style: {
        transform: "translateY(" + (props.item.index + 1) * 40 + "px)",
        zIndex: props.item.index
    }}, 
        React.createElement(SkillNameCell, {name: props.item.name, disappearance: props.item.disappearance}), 
        React.createElement(AddPowerCell, {power: props.item.plus}), 
        React.createElement("td", null, props.item.mult.toFixed(3)));
};
var SkillNameCell = function (props) {
    return React.createElement("td", null, 
        React.createElement("div", {className: "skillName"}, 
            React.createElement("span", null, props.name), 
            React.createElement("span", {className: "disappearance" + (props.disappearance ? '' : ' opacity0')}, props.disappearance ? ' - ' + props.disappearance : ''))
    );
};
var AddPowerCell = function (props) {
    var value = Math.min(Math.abs(props.power), 100);
    return React.createElement("td", null, 
        React.createElement("div", {className: "AddPower"}, 
            React.createElement("span", {className: "AddPower-graph" + (props.power < 0 ? ' minus' : ''), style: { transform: "scaleX(" + value / 100 + ")" }}), 
            React.createElement("span", {className: 'AddPower-value', style: { transform: "translateX(" + (value - 92) + "px)" }}, props.power))
    );
};
/// <reference path="weaponData.ts" />
var Weapon = (function (_super) {
    ...
    function Weapon(props) {
        ...
        var _a = getWeapon(type, this.state.name, this.state.level), power = _a[0], affinity = _a[1];
        this.props.setWeapon({ type: type, power: power, affinity: affinity });
    }
    ...
    Weapon.prototype.setType = function () {
        ...
        var level = getWeaponLevelList(type, name).length;
        var _a = getWeapon(type, name, level), power = _a[0], affinity = _a[1];
        this.setState({ name: name, level: level });
        this.props.setWeapon({ type: type, power: power, affinity: affinity });
    };
    ...
    Weapon.prototype.render = function () {
        ...
        if (this.props.weapon.power == null)
            return null;
        return React.createElement("section", {className: "Weapon"}, 
            React.createElement("h2", null, "Choose a Weapon"), 
            React.createElement("select", {ref: "type", className: "weapon-type", value: this.props.weapon.type, onChange: this.setType.bind(this)}, 
                React.createElement("option", {value: "lightbowgun"}, "ライト"), 
                React.createElement("option", {value: "heavybowgun"}, "ヘビィ")), 
            React.createElement("select", {ref: "name", className: "weapon-name", value: this.state.name, onChange: this.setName.bind(this)}, getWeaponList(this.props.weapon.type).map(function (value) {
            ...
            React.createElement("select", {ref: "level", className: "weapon-level", value: this.state.level + '', onChange: this.setLevel.bind(this)}, getWeaponLevelList(this.props.weapon.type, this.state.name).map(function (value, i) {
            ...
            React.createElement("p", {className: "weapon-power"}, this.props.weapon.power + " / " + this.props.weapon.affinity + "% => " + (power | 0)));
    };
    return Weapon;
}(React.Component));
...
var MHCalc = (function (_super) {
    ...
    MHCalc.prototype.setActiveSkill = function (skillGroup, skillName) {
    ...
    MHCalc.prototype.render = function () {
        return React.createElement("div", {className: "MHCalc"}, 
            ...
            React.createElement("section", null, 
                React.createElement("h2", null, "Skill Ranking"), 
